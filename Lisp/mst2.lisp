...
(defun graph-vertex-neighbors (graph-id vertex-id)
  (let* ((list)
	 (neighbors (progn
		      (maphash
		       (lambda (k v)
			 (push (list 'arc graph-id vertex-id k v) list))
...
(defun graph-vertex-adjacent (graph-id vertex-id)
  (let ((list))
    (mapcar (lambda (arc)
	      (push (list 'vertex graph-id (fourth arc)) list))
...
(defun mst-get-rec (graph-id source)
 ; (let ((right-symbol (if (not (null (gethash (list 'vertex graph-id source))))
;			  source
;			  ))))))
  (set-visited source)
...
(defun build-mst-graph (graph-id)
  (maphash (lambda (k v)
	     (let ((string-k (write-to-string (third k)))
		   (string-v (write-to-string (third v))))
...
(defun free-arcs (graph-id source)
  (let ((arcs-list))
    (maphash (lambda (k v)
	       (if (not (is-visited k))
		   (push  (list 'arc graph-id source (if (numberp k)
							 (write-to-string k)
							   k) v) arcs-list)))
...
(load "/Users/gabrielecanesi/Downloads/lisptest/primkiller_50k.LISP")
